...
def get_twp_counts():
    ...
    wcursor.execute(
        """select count(distinct(nri_id)), model_twp from combos 
        GROUP by model_twp"""
    )
    for row in wcursor:
    ...
    wcursor.execute(
        """select count(distinct(hrap_i)), model_twp from combos 
        GROUP by model_twp"""
    )
    for row in wcursor:
...
def main(year, month, day):
    ...
    wcursor.execute(
        """SELECT hrap_i, rainfall from daily_rainfall 
        WHERE valid = %s""",
        (ts,),
    )
    for row in wcursor:
    ...
    wcursor.execute(
        """SELECT run_id, valid, runoff, loss, precip from results 
        WHERE valid = %s""",
        (ts,),
    )
    rDict = {}
    ...
    for comboid in cDict.keys():
        ...
        if rDict.has_key(comboid):
            ...
            twpDict[model_twp]["max_runoff"] = max(
                ro, twpDict[model_twp]["max_runoff"]
            )
            twpDict[model_twp]["min_runoff"] = min(
                ro, twpDict[model_twp]["min_runoff"]
            )
            twpDict[model_twp]["max_loss"] = max(
                lo, twpDict[model_twp]["max_loss"]
            )
            twpDict[model_twp]["min_loss"] = min(
                lo, twpDict[model_twp]["min_loss"]
            )
        else:
        ...
        twpDict[model_twp]["max_precip"] = max(
            rainDict[hrap_i], twpDict[model_twp]["max_precip"]
        )
        twpDict[model_twp]["min_precip"] = min(
            rainDict[hrap_i], twpDict[model_twp]["min_precip"]
        )
    ...
    for model_twp in twpDict.keys():
        ...
        twpDict[model_twp]["avg_runoff"] = (
            twpDict[model_twp]["trunoff"] / truns
        )
        ...
        wcursor.execute(
            """INSERT into results_by_twp(model_twp, valid, 
    ...
    wcursor.execute(
        """DELETE from results_twp_year WHERE   
         valid = '%s-01-01'"""
        % (ts.year,)
    )
    ...
    wcursor.execute(
        """DELETE from results_twp_month WHERE 
         valid = '%s-%s-01'"""
        % (ts.year, ts.month)
    )
