    ...
    def buildSoil(self, mydb):
        ...
        try:
            o.write("'%s'  '%s' %s %s %s %s %s %s %s\n" % (rs[0]['name'], 
                    rs[0]['texture'], rs[0]['layers'], rs[0]['albedo'], 
                    rs[0]['sat'], rs[0]['interrill'], rs[0]['rill'], 
                    rs[0]['shear'], rs[0]['conduct']) )
        except:
            print "Missing Soil For NRI: %s , " % (self.nri_id,)
            self.error = 20
            return
        lys = mydb.query("""SELECT * from layers 
      WHERE soil_id = %s""" % (str(self.sid),) ).dictresult()
        for i in range(len(lys)):
            o.write("%s %s %s %s %s %s\n" % (lys[i]['depth'], 
                    lys[i]['sand'], lys[i]['clay'], lys[i]['om'], 
                    lys[i]['cec'], lys[i]['rock']) )
...
%s
0
""" % ("wepp.out", "wb/"+ str(self.run_id)+".wb", 
