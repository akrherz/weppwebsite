...
ets = mx.DateTime.now() + mx.DateTime.RelativeDateTime(
...
class WeppThread(threading.Thread):
    ...
    def chunk(self):
        ...
        rs = self.dbconn.query(
            "SELECT \
             nextval('job_queue_request_id'::text) as next"
        ).dictresult()
        requestID = rs[0]["next"]

        # Send a request to the server for 1000 combos to make
        self.dbconn.query(
            "UPDATE job_queue SET request_id = %s \
        ...
        sql = (
            "select c.mkrun as mkrun, c.id as cid, c.hrap_i, \
               n.steep, n.len, \
               ...
               and c.nri_id = n.id and n.man_id = m.man_id"
            % (requestID)
        )
