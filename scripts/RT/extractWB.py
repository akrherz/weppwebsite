...
if len(sys.argv) == 4:
    ets = datetime.datetime(int(sys.argv[1]),int(sys.argv[2]),int(sys.argv[3]))
...
wcursor.execute("insert into waterbalance_by_twp (select '%s', model_twp, avg(vsm), stddev(vsm), 0, avg(s10cm), avg(s20cm), avg(et) from waterbalance, combos WHERE combos.id = waterbalance.run_id and waterbalance.valid = '%s' GROUP by model_twp)"%(d,d))
