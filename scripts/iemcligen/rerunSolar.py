...
cref = {1: 'a138019', 2: 'a134309', 3: 'a135879', 
        4: 'a131299', 5: 'a130209', 6: 'a131329',
        7: 'a134759', 8: 'a131559', 9: 'a131909'}
...
while (now < e):
  ...
  for sector in cref.keys():
    ...
    rs = mydb.query("SELECT c80 from "+ tbl +" WHERE \
     day = '"+ day +"' ").dictresult()
    if (len(rs) > 0 and float(rs[0]['c80']) > 0):
      rad = str(rs[0]['c80'])
      last_value = rad
    else:
      print "Using old value %s for date %s" % ( last_value, day)
      rad = last_value
    print "last_value is %s" % ( last_value)
    last_day = (now - mx.DateTime.RelativeDateTime(years=4)).strftime("%Y-%m-%d")
    wepp.query("UPDATE climate_sectors SET rad = '"+ rad +"' \
