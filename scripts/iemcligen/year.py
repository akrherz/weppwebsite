...
def createNETCDF(s):
	...
	r15m = nc.createVariable("rainfall_15min", Numeric.Float, ('time', 'hrap_j', 'hrap_i') )
	r15m.units = "mm"
	r1d = nc.createVariable("rainfall_1day", Numeric.Float, ('hrap_j', 'hrap_i'))
	r1d.units = "mm"
...
def main(_YEAR, _MONTH, _DAY):
	...
	for row in range(len(rain)):
		for col in range(len(rain[row])):
			...
			if (rain[row][col] > 0):
				sql.write("%s\t%s\t%s\t%s\t%s\n" % \
	(i +1, stringDate, rain[row][col], max_rain15[row][col], hr_cnt[row][col]))
