    ...
    for i in range(4):
        ...
        for s in storms[i]:
            ...
            sql = """UPDATE events SET precip = %.2f, onset = '%s', duration = %s 
                    WHERE storm_id = %s and basin_id = %s""" % (rain, 
                        sts.strftime('%Y-%m-%d %H:%M'), duration, sid, basin)
            scursor.execute( sql )
...
if __name__ == '__main__':
    ...
    basins = [11829, 11656, 11831, 11658, 11659, 11313, 11139, 11141, 11488, 
              10968, 11316, 10796, 10797]
