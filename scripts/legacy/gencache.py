...
while now < e:
    ...
    for prod in ("rainfall_in", "avg_loss_acre", "avg_runoff_in", "vsm"):
        url = (
            "http://wepp.mesonet.agron.iastate.edu/GIS/plot.php?duration=daily&width=320&height=240&var=%s&dstr=%s"
            % (prod, now.strftime("%m/%d/%Y"))
        )
        cmd = "wget -q -O %s/%s_daily_%s.png '%s'" % (
            dir,
            now.strftime("%d"),
            prod,
            url,
