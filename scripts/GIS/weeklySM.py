...
twp = {}
rs = mydb.query(
    "SELECT astext(transform(the_geom,4326)) as t, model_twp "
    "from iatwp ORDER by model_twp ASC"
).dictresult()
...
while now < ets:
    shp = shapelib.create(
        "weeklysm/%ssm" % (now.strftime("%Y%m%d"),), shapelib.SHPT_POLYGON
    )
    ...
    shutil.copy(
        ...
        "weeklysm/%ssm.prj" % (now.strftime("%Y%m%d"),),
    )
