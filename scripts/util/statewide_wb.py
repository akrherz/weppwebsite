...
WEPP = psycopg2.connect(database='wepp', host='mesonet.agron.iastate.edu', user='nobody')
cursor = WEPP.cursor()
...
for i, year in enumerate(range(1997,2014)):
    cursor.execute("""
     ...
     GROUP by valid ORDER by valid ASC""" % (year, year))
    ...
    if highlights.has_key(year):
        ax.plot(x, y, label='%s' % (year,), c=highlights[year], zorder=2, lw=2.0)
    else:
        ax.plot(x, y, c='tan')        
    if year == 2011:
        x2 = x
...
ax.set_title("Iowa Daily Erosion Project (WEPP Model) Volumetric Soil Moisture\n(1997-2013) Iowa Areal Averaged Root Zone Soil Moisture")
